...
### 2.5 什么是 QPS/RPM/TPM？—— API 限流的"计量单位"

**一句话定义：** 限流是因为 API 供应商的服务器能力有限，它用三种单位来衡量你能用多少：QPS（每秒请求数）、RPM（每分钟请求数）、TPM（每分钟 Token 数）。

**三种限流单位对比：**
| 单位 | 中文 | 含义 | 场景 |
|:----|:----|:-----|:----|
| **QPS** | 每秒查询数 | 每秒能发多少请求 | API 调用频率限制（一般 10-100 QPS）|
| **RPM** | 每分钟请求数 | 每分钟能发多少请求 | 长期的配额限制 |
| **TPM** | 每分钟 Token 数 | 每分钟能处理多少 Token | 计算资源限制（最严格）|

**为什么限流不用"次数"而用"Token"？**
```
两个请求，资源消耗天差地别：

请求A："你好" → 总共 10 Token → 处理时间 ≈ 0.05 秒
请求B："写一篇 5000 字的文章..." → 总共 6000 Token → 处理时间 ≈ 3 秒

如果只按次数限流：
  请求A 和 请求B 都算"1 次" → 不公平，请求B 占了 60 倍资源

如果按 Token 限流：
  请求A = 10 TPM，请求B = 6000 TPM → 合理，谁用得多谁扣得多
```
...
qps = (users * avg_dialogs) / peak_hours * peak_factor
print(f"估算 QPS: {qps:.0f}")  # ≈ 52 QPS
```

**面试话术：**
> "限流不是 API 的 bug，是 API 的 feature。关键是你怎么应对。我见过生产环境被限流后直接死循环重试——每秒重试 100 次同一个 Key，越限越重试，越重试越限。正确的做法是多 Key 轮换（每个 Key 承担部分压力）+ 指数退避（不要拼命重试）+ 监控告警（限流比例超过阈值自动通知）。"

---

### 2.6 指数退避（Exponential Backoff）—— 重试的科学

**一句话定义：** 指数退避是一种"越等越久"的重试策略——第一次失败等 1 秒，第二次等 2 秒，第三次等 4 秒，第四次等 8 秒... 而不是每次都等固定时长。

**三种重试策略对比：**

```
场景：API 限流了 429，10 个客户端同时在重试

策略 A：固定间隔 1 秒重试
  客户端 1-10：都在第 1 秒重试 → 全失败（还是 429）
  客户端 1-10：都在第 2 秒重试 → 全失败
  → 产生"重试风暴"，服务器越来越忙

策略 B：指数退避 1s → 2s → 4s → 8s
  客户端 1：1s → 2s → 4s → 8s
  客户端 2：1s → 2s → 4s → 8s
  → 重试间隔拉开，压力分散

策略 C：指数退避 + 随机抖动（Jitter）
  客户端 1：0.8s → 2.3s → 3.7s → 8.5s
  客户端 2：1.3s → 1.8s → 4.2s → 7.6s
  → 加上随机值后重试时间完全错开，效果最好
```

**指数退避公式：**
```
wait = min(max_wait, base_delay × 2^attempt) + random(0, jitter)

实际例子：
  base_delay = 1 秒, max_wait = 60 秒, jitter = 0.5 秒

  第 1 次重试：wait = min(60, 1×2¹) + rand(0, 0.5) ≈ 2.3 秒
  第 2 次重试：wait = min(60, 1×2²) + rand(0, 0.5) ≈ 4.1 秒
  第 3 次重试：wait = min(60, 1×2³) + rand(0, 0.5) ≈ 8.2 秒
  第 4 次重试：wait = min(60, 1×2⁴) + rand(0, 0.5) ≈ 16.3 秒
  第 5 次重试：wait = min(60, 1×2⁵) + rand(0, 0.5) ≈ 32.1 秒
  第 6 次重试：wait = min(60, 1×2⁶) + rand(0, 0.5) ≈ 60.0 秒（封顶）
```
...
请求失败
  ...
  │     └── 所有 Key 都失败 → 触发降级
  │
  └── 是网络错误？→ 指数退避重试（网络通常是临时问题）
```

**面试话术：**
> "指数退避 + 随机抖动是生产环境的标准配置。我见过一个系统上线第一天就崩溃了——因为 100 个客户端同时发现 API 超时，都在 1 秒后重试，形成了重试风暴。加了抖动后，客户端重试时间错开，服务就稳定了。Key 池和指数退避是双保险：先退避重试同一个 Key，连续失败才切 Key——这样既避免了浪费其他 Key 的配额，又不会出现无休止的重试。"
